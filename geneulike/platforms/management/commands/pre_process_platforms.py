...
def extract_info(organism):
    ...
    max_data = 500
    organism_name = organism.name.replace(" ", "+")
    ncbi_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/esearch.fcgi?db=gds&term=GPL[ETYP]{}[Organism]&retmax={}&usehistory=y&retmode=json".format(organism_name, str(max_data))
    ...
    _process_data(data["esearchresult"]['querykey'], data["esearchresult"]['webenv'], organism, max_data)
    current_count = max_data
    i = 0
    while i < iterations:
        _process_data(data["esearchresult"]['querykey'], data["esearchresult"]['webenv'], organism, max_data, current_count)
        current_count += max_data
        i += 1

def _process_data(query_key, web_env, organism, max_data=500, current_count=0):
    ...
    for id, platform in data['result'].items():
        ...
        dict_data = {
            ...
            "geo_accession": platform["accession"],
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for specie in species:
            print("Parsing platforms for species " + specie.name)
            extract_info(specie)
